...
-- __CLIENTE__. Generar el SQL de una marca con
...
create schema if not exists __CLIENTE__;

create table __CLIENTE__.documentos_conocimiento (
...
create index on __CLIENTE__.documentos_conocimiento using hnsw (embedding vector_cosine_ops);
create index on __CLIENTE__.documentos_conocimiento using gin (contenido_fts);
...
create or replace function __CLIENTE__.match_documentos(
...
as $$
    with semantico as (
        ...
        from __CLIENTE__.documentos_conocimiento
    ...
    from __CLIENTE__.documentos_conocimiento d
...
grant usage on schema __CLIENTE__ to anon, authenticated, service_role;
grant all privileges on table __CLIENTE__.documentos_conocimiento to service_role;
grant execute on function __CLIENTE__.match_documentos(vector, text, int, text) to anon, authenticated, service_role;
...
alter table __CLIENTE__.documentos_conocimiento enable row level security;
